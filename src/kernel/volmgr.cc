...
bool volmgr_t::witness(const uuids& uuid, int64_t* kpi)
{
...
void volmgr_t::set_initusn(int64_t kpi, uint64_t usn)
{
	sql_stmt stmt = dbmgr_->create_stmt_ex(
			UT("UPDATE known_ntfs SET init_usn=$2 WHERE id=$3;"));
	stmt.bind(2, usn);
	stmt.bind(3, kpi);
	stmt.execute();
}

void volmgr_t::update_ntfsext(int64_t kpi, uint64_t usn)
{
	sql_stmt stmt = dbmgr_->create_stmt_ex(
			UT("UPDATE known_ntfs SET usn_pointer=$2 WHERE id=$3;"));
	stmt.bind(2, usn);
	stmt.bind(3, kpi);
	stmt.execute();
...
{
	int64_t init_usn, usn_read, usn_next;	
	int64_t sum_usn_read = 0, sum_usn_next = 0;	
	sql_stmt stmt = dbmgr_->create_stmt_ex(
		UT("SELECT init_usn, usn_pointer, usn_next FROM known_ntfs;"));
	while (stmt.step())
	{
		stmt.col(1, init_usn);
		stmt.col(2, usn_read);
		stmt.col(3, usn_next);
		sum_usn_read += usn_read - init_usn;
		sum_usn_next += usn_next - init_usn;
	}
